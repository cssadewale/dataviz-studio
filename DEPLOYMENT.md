# Tableau Stimulator — Deployment Guide
...
After deployment, click the topbar badge **“by Adewale Samson Adeagbo”**. Confirm that the About modal displays the portfolio, HMG Concepts link, GitHub, LinkedIn, YouTube, WhatsApp and email details.

Seven methods for deploying Tableau Stimulator, from the simplest single-device use
...
3. Repository name: `tableau-stimulator`
...
2. Select and upload all files at once:
   - `index.html` (rename from tableau-stimulator-v7.html)
   - `README.md`
   - `LICENSE`
   - `CHANGELOG.md`
   - `CONTRIBUTING.md`
   ...
   - `CODE_OF_CONDUCT.md`
   - `.gitignore`
3. Commit message: `feat: initial release — Tableau Stimulator v7`
...
3. A green banner appears: "Your site is live at https://cssadewale.github.io/tableau-stimulator/"
...
3. Set:
   - Website: `https://cssadewale.github.io/tableau-stimulator/`
...
- Type e.g. `tableau-stimulator-adewale`
- Your URL becomes: `https://tableau-stimulator-adewale.netlify.app`
...
# Project name: tableau-stimulator
# Directory: ./
# Override settings? No
```

Your site deploys at `https://tableau-stimulator.vercel.app`
...
Every student or colleague on the same network can access Tableau Stimulator in their browser.
...
4. The browser opens and Tableau Stimulator runs completely offline from the USB drive
...
## Clear Production Deployment Checklist for Tableau Stimulator (Recommended Path)

Use this checklist when you want a clean public deployment using only free tools.

### A. Prepare the files

1. Keep `index.html` in the project root. It is the whole application.
2. Keep documentation files beside it: `README.md`, `DEPLOYMENT.md`, `SAMPLE_DATA.md`, `SECURITY.md`, `CHANGELOG.md`, `CONTRIBUTING.md`, `LICENSE.txt`.
3. Do not add paid API keys. The app does not need a backend, database, AI API, Node build step, or server code.
4. Open `index.html` locally in Chrome and confirm that the topbar loads and there is no browser console syntax error.
5. Upload a small CSV and test: Worksheet render, Pivot build, Statistics, Data Cleaner, Dashboard, Report export, Save/Load workspace.

### B. Deploy to GitHub Pages

1. Create a free GitHub account or log in.
2. Create a public repository, for example `tableau-stimulator`.
3. Upload all project files to the repository root. The file must be named exactly `index.html`.
...
9. Click **Save**.
10. Wait 2–5 minutes.
11. Open the displayed URL, normally `https://YOUR-USERNAME.github.io/tableau-stimulator/`.
12. Hard refresh once (`Ctrl+Shift+R`) if an older cached version appears.

### C. Deploy to Netlify Drag-and-Drop

1. Go to `https://www.netlify.com/` and sign in with a free account.
2. On the dashboard, choose **Add new site** → **Deploy manually**.
3. Drag the whole project folder, or at minimum `index.html`, into the upload area.
4. Netlify immediately provides a public URL.
5. Optional: go to **Site configuration** → **Change site name** and choose a readable free subdomain.

### D. Validation after deployment

1. Open the deployed URL in Chrome, Edge and Firefox if available.
2. Upload a sample CSV.
3. Build a bar chart using COLS and ROWS.
4. Test the v8 enhancements: Forecast, Moving Average, Waterfall/Funnel/Gauge, Presentation Mode.
5. Export a PNG and a report HTML.
6. Save a workspace JSON, reload the page, and load the JSON back.
7. Confirm no dataset leaves the browser. This is client-side only.

### E. Updating the live version

1. Edit `index.html` locally.
2. Test locally first.
3. Commit and push to GitHub, or re-upload to Netlify.
4. Wait for redeployment.
5. Hard refresh the public URL.
